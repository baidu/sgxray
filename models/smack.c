 ...
 * ---------                                        ----------
 * A > 0 && A < $ENCLAVE_BASE                       Outside Memory
 * A >= $ENCLAVE_BASE && A < $MALLOC_TOP            Enclave Memory
...
void __SMACK_decls(void) {
  ...
  D("const $ENCLAVE_BASE: ref;\n");

#if MEMORY_SAFETY
#if SMACK_SGX_ENABLED
  __SMACK_dummy((long long)__SMACK_check_memory_safety);
  D("implementation __SMACK_check_memory_safety(p: ref, size: ref)\n"
    "{\n"
    "  assume size > 0;\n"
#if SMACK_ONLY_CHECK_ENCLAVE_MEMORY
#if SMACK_ENABLE_NULL_CHECK
    "  if (!($sgt.ref.bool(p, $0.ref) &&"
#else
    "  if (!($sge.ref.bool(p, $0.ref) &&"
#endif
    " $slt.ref.bool($add.ref(p, size), $ENCLAVE_BASE))) {\n"
#endif
  ...
  D("procedure $malloc(n: ref) returns (p: ref)\n"
    ...
    "{\n"
#if SMACK_ENABLE_FAILING_MALLOC
    "  if (*) {\n"
    "    p := $0.ref;\n"
    "    return;\n"
    "  }\n"
...
#if SMACK_SGX_ENABLED
  D("var $EnclaveCurrAddr:ref;\n");
  D("procedure {:inline 1} $$alloc(n: ref) returns (p: ref)\n"
    "modifies $Alloc, $EnclaveCurrAddr;\n"
    "{\n"
    "  assume $sle.ref.bool($0.ref, n);\n"
    "  if ($slt.ref.bool($0.ref, n)) {\n"
    "    p := $EnclaveCurrAddr;\n"
    "    havoc $EnclaveCurrAddr;\n"
    "    assume $sge.ref.bool($sub.ref($EnclaveCurrAddr, n), p);\n"
    "    assume $sge.ref.bool($EnclaveCurrAddr, $ENCLAVE_BASE) && "
    "$slt.ref.bool($EnclaveCurrAddr, "
    "$MALLOC_TOP);\n"
    "    assume $Size(p) == n;\n"
    ...
    "  }\n"
    "}\n");
  D("procedure {:inline 1} $sgx_ocalloc(n: ref) returns (p: ref)\n"
    "modifies $CurrAddr;\n"
    "{\n"
