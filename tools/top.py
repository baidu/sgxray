...
class VResult(Flag):
    ...
    def message(self, args, err_loc=None):
        ...
        if self is VResult.VERIFIED:
            ...
            return ('SMACK found an error'
                    ...
                    + ('' if err_loc is None else ' at %s' % err_loc)
                    + '.')
...
def arguments():
    ...
    frontend_group.add_argument(
        ...
        nargs='*',
        default=[],
        help='Intel SGX SDK static library bitcode files')
    ...
    translate_group.add_argument(
        ...
        action='store_true',
        default=False,
        help='enable support for sgx functions')

    translate_group.add_argument(
        '--havoc-outside-memory',
        action='store_true',
        default=False,
        help='assume non-deterministic behaviors of memory outside enclaves')

    translate_group.add_argument(
        '--only-check-enclave-memory',
        action='store_true',
        default=False,
        help='only check memory safety for memory within an enclave')

    translate_group.add_argument(
        '--enable-null-check',
        action='store_true',
        default=False,
        help='''Check memory safety for memory within an enclave as well as null
                pointer deferences inside enclaves''')

    translate_group.add_argument(
        '--enable-failing-malloc',
        action='store_true',
        default=False,
        help='''Assume that malloc can return a null pointer
...
def llvm_to_bpl(args):
    ...
    if args.modular:
        cmd += ['-modular']
    if args.havoc_outside_memory:
        cmd += ['-havoc-outside-memory']
    if args.ecall_name:
        cmd += ['-ecall-name=' + args.ecall_name]
...
def verify_bpl(args):
    ...
    if args.smackd:
        ...
        if result in VResult.ERROR:
            error, err_loc = error_trace(verifier_output, args.verifier)
        ...
        print(result.message(args, err_loc))
