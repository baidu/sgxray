...
def default_clang_compile_command(args, lib=False):
    ...
    if args.integer_encoding == 'bit-vector':
        cmd += ['-DBIT_PRECISE']
    if args.sgx:
        cmd += ['-DSMACK_SGX_ENABLED']
        cmd += ['-DCUSTOM_VERIFIER_ASSERT']
    if args.only_check_enclave_memory:
        cmd += ['-DSMACK_ONLY_CHECK_ENCLAVE_MEMORY']
    if args.enable_null_check:
        cmd += ['-DSMACK_ENABLE_NULL_CHECK']
    if args.enable_failing_malloc:
        cmd += ['-DSMACK_ENABLE_FAILING_MALLOC']
...
def default_build_libs(args):
    ...
    bitcodes = []
    libs = ['smack.c', 'stdlib.c', 'errno.c', 'smack-rust.c', 'stdio.c']

    if args.pthread:
        libs += ['pthread.c']

    if args.strings or args.sgx:
        libs += ['string.c']
...
def link_bc_files(bitcodes, libs, args):
    ...
    smack_libs = [o for lib in smack_libs for o in ['--override', lib]]
    bitcodes += args.sgx_sdk_libs
